%spikemaster - reproduce Fig. S???
%
%   spikemaster simulates the spiking two-node model
%   doing the two-interval discrimination for a fixed
%   set of stimulus combinations (f1,f2).
%   [ (f1,f2) = (1,2) (2,1) (2,3) (3,2) ... (6,7( (7,6) ]
%   The neuron, synapse, and network parameters are taken
...
%ftrial = [ftrial; ftrial; ftrial; ftrial; ftrial];
...
for l=1:size(ftrial, 1 )
  ...
  [sx,sy,t] = spikesim( f1, f2 );
  
  %plot this trace
  pp = plot( t(t<par.TSO2+200),  1000*sx(t<par.TSO2+200) );
  set( pp, 'Color', rcs(f1,:) );
  pp = plot( t(t>=par.TSO2+200), 1000*sx(t>=par.TSO2+200) );
  if ( f1>f2 )
