function [u, w, du, dw] = SigExpLinearModel(t,u0,w0,A,B,I)
%% SigExpLinearModel
%
...
for ti = 1:length(t)
    if ti == 1
        eta = B*randn(2,1);
        du(ti) = ( A(1,1)*( u0./(1+u0.^2) ) +...
            A(1,2)*( exp(w0) ) + I(1,ti) + eta(1) )*dt;
        dw(ti) = ( A(2,1)*( u0./(1+u0.^2) ) +...
...
%             A(2,2)*( exp(w0) ) + I(2,ti) + eta(2) )*dt;
        ...
        eta = B*randn(2,1);
        du(ti) = ( A(1,1)*( u(ti-1)./(1+u(ti-1).^2) ) +...
            A(1,2)*( exp(w(ti-1)) ) + I(1,ti) + eta(1) )*dt;
        dw(ti) = ( A(2,1)*( u(ti-1)./(1+u(ti-1).^2) ) +...
