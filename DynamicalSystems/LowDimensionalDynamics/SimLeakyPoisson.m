function SimLeakyPoisson(t,N,varargin)
%%
%
%
%
...
for ti = 2:length(t)
    dv(:,ti,:) = ( -v(:,ti-1,:) + repmat(B,[1,1,size(u,3)]).*u(:,ti,:) + ...
        permute(Sigma*randn(N,length(coherence)),[1,3,2]) )/tau;
...
function u = setUdefault(t,coherence,N)
    ...
    utemp = repmat(permute(coherence,[1,3,2]),[N,sum(t>=0 & t<=100),1]);
    u(:,t >= 0 & t <= 100,:) = utemp;
