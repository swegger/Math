...
w0 = 0.5 + log(ts/max(ts));                % Initial condition of w
...
U = X1(:);
W = X2(:);
[dU, dW, uNull, wNull] = SigExpLinearModelVectorField(U,W,A{end},I);

%% Run simulation and "kick" the partical to a new position at different times
for i = 1:length(ts)
    % Run the model up until the kick
    t{i} = 0:dt:ts(i);
    [u{i}, w{i}, du{i}, dw{i}] = SigExpLinearModel(t{i},u0(i),w0(i),A{i},B,I*sqrt(max(ta)/ta(i)));
...
figure('Name','Input')
for i = 1:length(ts)
    plot(t{i},I(1,1:length(t{i}))*sqrt(max(ta)/ta(i)),'Color',colors(i,:),'LineWidth',2)
    hold on
end
xlabel('Time (s)')
ylabel('Input');
