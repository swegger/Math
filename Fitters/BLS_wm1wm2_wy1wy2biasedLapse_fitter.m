function [wm, wy, wy2, wm_drift, b, lapse, llikelihood, lmodelEvidence] = ...
    BLS_wm1wm2_wy1wy2biasedLapse_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    minimizer = 'fmincon(minimizant, [wM_ini wP_ini wP2_ini wM_drift_ini b_ini lapse_ini], [], [], [], [], lb, ub, [], OPTIONS);';
    if ii == 1
        wM_ini = IC(1);
        wP_ini = IC(2);
        wP2_ini = IC(3);
...
function logL = logLikelihoodQUAD(wm,wy,wy2,wm_drift,b,lapse,N,x,y,xmin,xmax,dx,M,m,pmin,pmax,ObsAct)
%% LOGLIKELIHOODQUAD
%
...
if iscell(N)
    ...
    for i = 1:length(N)
        ...
        WY = repmat(permute(wyn(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
