function [theta, logPosterior, exitflg, output, thetas] = FitMultivariateLinearExpPoissonObsMAP(x,y,theta_init,varargin)
%% FitMultivariateLinearExpPoissonMAP
%
%   theta = FitMultivariateLinearPoissonObsMAP(x,y,theta_init)
%
%   Fits a linear model to data with Poisson observation noise. Model assumes
%   the following form:
%       p(y|x) = (lambda^y*exp(-lambda))/(y!)
%       lambda = exp( theta'*[x ones(1,size(x,2))] );
...
for i = 1:validation.folds
    % Partition data
    fitinds = false(size(x,2),1);
    fitinds((i-1)*floor(size(x,2)/validation.folds)+1:(i)*floor(size(x,2)/validation.folds)) = true;
    xfit = x(:,fitinds);
    yfit = y(fitinds);
...
p = p(:);
xaugmented = [x; ones(1,size(x,2))];
lambda = @(x)( exp(p'*xaugmented) );
