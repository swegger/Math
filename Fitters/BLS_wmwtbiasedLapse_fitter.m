function [wm, wy, wt, b, lapse, llikelihood, lmodelEvidence] = ...
    BLS_wmwtbiasedLapse_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    minimizer = 'fmincon(minimizant, [wM_ini wP_ini wT_ini b_ini lapse_ini], A, C, [], [], lb, ub, [], OPTIONS);';
    if ii == 1
        wM_ini = IC(1);
        wP_ini = IC(2);
        wT_ini = IC(3);
...
error('BLS_wmwt model not yet supported for FitType = "integral"')
...
error('BLS_wmwt model not yet supported for FitType = "trapz"')

function logL = logLikelihoodQUAD(wm,wy,wt,b,lapse,N,x,y,xmin,xmax,dx,M,m,pmin,pmax,ObsAct)
%% LOGLIKELIHOODQUAD
%
...
if iscell(N)
    ...
    for i = 1:length(N)
        ...
        WT = repmat(permute(wm_drift(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
...
%             p_m_take_x = (1./sqrt(2.*pi.*WM.^2.*X.^2)).^n .* exp( -squeeze(sum((repmat(permute(M(1:l^n,1:n,:),[1 4 2 3]),[1 size(X,2) 1 1])-repmat(permute(X,[1 2 4 3]),[1 1 n 1])).^2,3))./(2.*WM.^2.*X.^2) );
            p_m1_take_x = (1./sqrt(2.*pi.*WT.^2.*X.^2)) .* ...
                exp( -squeeze(...
...
%     end
    ...
    fBLS = repmat(permute(fBLS,[1 3 2]),[1,size(X,2), 1]);
    WM = repmat(permute(wm(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
    WT = repmat(permute(wt(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
    WY = repmat(permute(wy(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
    B = repmat(permute(b(:),[2 3 1]),[size(fBLS,1), size(X,2), 1]);
    ...
    if N == 1
        p_m_take_x = (1./sqrt(2.*pi.*WM.^2.*X.^2)) .* ...
            ...
            (2.*WM.^2.*X.^2) );
    elseif N == 2
        p_m1_take_x = (1./sqrt(2.*pi.*(WM.^2 + WT.^2).*X.^2)) .* ...
            exp( -squeeze(...
            (repmat(permute(M(1:l^N,1,:),[1 4 2 3]),[1 size(X,2) 1 1])...
            -repmat(permute(X,[1 2 4 3]),[1 1 1 1])).^2) ./...
            (2.*WT.^2.*X.^2) );
...
error('BLS_wmwt model not yet supported for FitType = "quad_batch"')

function logL = logLikelihoodQUADnested(wm,wy,b,N,x,y,xmin,xmax,dx,m,batchsize,ObsAct)


error('BLS_wmwt model not yet supported for FitType = "quad_nested"')
