function [wm, wy, wm_drift, w_int, b, lapse, alpha, llikelihood, lmodelEvidence] = ...
    NestedModelbiasedLapse_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    switch FitType
        ...
        case 'quad'
            ...
            minimizant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),p(:,6),p(:,7),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
            validant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),p(:,6),p(:,7),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
        ...
        case 'quad_batch'
            ...
            minimizant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),p(:,6),p(:,7),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize,ObsAct);
            validant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),p(:,6),p(:,7),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize,ObsAct);
...
function logL = logLikelihoodQUAD(wm,wy,wm_drift,w_int,b,lapse,alpha,N,x,y,xmin,xmax,dx,M,m,pmin,pmax,ObsAct)
%% LOGLIKELIHOODQUAD
%
