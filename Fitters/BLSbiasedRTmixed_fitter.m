function [wm, wy, b, mid, s, mu, sig, llikelihood] = BLSbiasedRTmixed_fitter(x,y,varargin)
...
switch FitType
    ...
    case 'quad'
        ...
        BLSminimizant = @(p)logLikelihoodQUAD(p(1),p(2),p(3),p(4),p(5),p(6),p(7),N,x,y,xmin,xmax,dx);
        
end

minimizer = 'fminsearch(BLSminimizant, [wM_ini wP_ini b_ini mid_ini s_ini mu_ini sig_ini], OPTIONS);';
wM_ini = IC(1);
wP_ini = IC(2);
b_ini = IC(3);
mid_ini = IC(4);
...
function logL = logLikelihoodQUAD(wm,wy,b,mid,s,mu,sig,N,x,y,xmin,xmax,dx)
%% LOGLIKELIHOODQUAD
%
...
logistic = @(m,mid,s)( 1 ./ ( 1 + exp(-(m-mid)/s) ) );
%rt_dist = @(y,mu,sig)( 1./(y.^2) .* (1./sqrt(2*pi*sig^2)) .* exp( -(y-mu).^2./(2*sig.^2) ) );
rt_dist = @(y,mu,sig)( (1./sqrt(2*pi*sig^2)) .* exp( -(y-mu).^2./(2*sig.^2) ) );       % Gaussian RT distribution
...
integrand = logistic(repmat(M(:,1),[1 size(X,2) 1]),mid,s).*p_y_take_fBLS.*p_m_take_x + (1-logistic(repmat(M(:,1),[1 size(X,2) 1]),mid,s)).*rt_dist(Y,mu,sig).*p_m_take_x;
