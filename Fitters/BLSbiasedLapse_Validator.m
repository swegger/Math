function [llikelihood, lmodelEvidence] = BLSbiasedLapse_Validator(x,y,wm,wy,b,lapse,varargin)
...
for ii = 1:length(x)
    switch FitType
        ...
        case 'quad'
            ...
            BLSvalidant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),N,x,y,xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2));
        ...
        case 'quad_batch'
            ...
            BLSvalidant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),p(:,4),N,x,y,xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize);
