function [wm, wy, b, lapse, llikelihood, lmodelEvidence] = EKF_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    switch FitType
        ...
        case 'quad'
            ...
            minimizant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
            validant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
