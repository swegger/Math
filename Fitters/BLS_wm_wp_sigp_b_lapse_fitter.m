function [wm, wy, b, lapse, sig, llikelihood, lmodelEvidence] = BLS_wm_wp_sigp_b_lapse_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    switch FitType
        ...
        case 'quad'
            ...
            BLSminimizant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2));
            BLSvalidant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2));
        ...
        case 'quad_batch'
            ...
            BLSminimizant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize);
            BLSvalidant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize);
    ...
    lb = [0 0 -Inf 0 0];
    ub = [1 1 Inf 1 Inf];
    minimizer = 'fmincon(BLSminimizant, [wM_ini wP_ini b_ini lapse_ini sig_ini], [], [], [], [], lb, ub, [], OPTIONS);';
    if ii == 1
        wM_ini = IC(1);
        wP_ini = IC(2);
        b_ini = IC(3);
