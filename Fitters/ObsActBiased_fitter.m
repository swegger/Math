function [wm, wy, b, llikelihood, lmodelEvidence] = ObsActBiased_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    switch FitType
        ...
        case 'quad'
            ...
            BLSminimizant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2));
            BLSvalidant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2));
        ...
        case 'quad_batch'
            ...
            BLSminimizant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize);
            BLSvalidant = @(p)logLikelihoodQUADbatch(p(:,1),p(:,2),p(:,3),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),batchsize);
...
function logL = logLikelihoodQUAD(wm,wy,b,N,x,y,xmin,xmax,dx,M,m,pmin,pmax)
%% LOGLIKELIHOODQUAD
%
