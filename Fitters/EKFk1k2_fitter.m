function [wm, wy, b, lapse, K, llikelihood, lmodelEvidence] = EKFk1k2_fitter(x,y,varargin)
...
for ii = 1:length(xfit)
    ...
    switch FitType
        ...
        case 'quad'
            ...
            minimizant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5:6),N,xfit{ii},yfit{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
            validant = @(p)logLikelihoodQUAD(p(:,1),p(:,2),p(:,3),p(:,4),p(:,5:6),N,xval{ii},yval{ii},xmin,xmax,dx,M,m,LapseSupport(1),LapseSupport(2),ObsAct);
    ...
    minimizer = 'fmincon(minimizant, [wM_ini wP_ini b_ini lapse_ini K_ini], [], [], [], [], lb, ub, [], OPTIONS);';
    if ii == 1
        wM_ini = IC(1);
        wP_ini = IC(2);
        b_ini = IC(3);
    ...
    try
        ...
        save('/om/user/swegger/EKFk1k2_fitterERROR')
        rethrow(ME)
    end
...
function logL = logLikelihoodQUAD(wm,wy,b,lapse,K,N,x,y,xmin,xmax,dx,M,m,pmin,pmax,ObsAct)
%% LOGLIKELIHOODQUAD
%
...
%     integrand = p_y_take_f.*p_m_take_x;
    ...
    M = repmat(permute(M,[1,3,4,2]),[1 size(X,2) length(wm) 2]);
