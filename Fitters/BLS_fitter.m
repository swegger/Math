function [wm, wy, b, llikelihood] = BLS_fitter(x,y,varargin)
...
switch FitType
    case 'integral'
        % Use integral
        BLSminimizant = @(p)logLikelihood(p(1),p(2),N,x,y,xmin,xmax);
        
    case 'trapz'
        % Use trapz
        BLSminimizant = @(p)logLikelihoodTRAPZ(p(1),p(2),N,m,x,y);
    ...
    case 'quad'
        ...
        BLSminimizant = @(p)logLikelihoodQUAD(p(1),p(2),N,x,y,xmin,xmax,dx,M,m);
    ...
    case 'quad_batch'
        ...
        BLSminimizant = @(p)logLikelihoodQUADbatch(p(1),p(2),N,x,y,xmin,xmax,dx,M,m,batchsize);
    ...
    case 'quad_nested'        
        ...
        BLSminimizant = @(p)logLikelihoodQUADnested(p(1),p(2),N,x,y,xmin,xmax,dx,m,batchsize);
end
...
function logL = logLikelihood(wm,wy,N,x,y,xmin,xmax)
%% LOGLIKELIHOOD
%
...
switch N
    case 1
        ...
        likefunction = @(x,y,m,xmin,xmax,wm,wy)(1./wy./wm./x./fBLS(m,xmin,xmax,wm).*exp(-1/2.*(y-(fBLS(m,xmin,xmax,wm))).^2./(wy.*fBLS(m,xmin,xmax,wm)).^2).*exp(-1/2.*(m-x).^2./(wm.*x).^2));
    ...
    case 2
        ...
        likefunction = @(x,y,m1,m2,xmin,xmax,wm,wy)(1./wy./wm.^2./x.^2./fBLS(m1,m2,xmin,xmax,wm).*exp(-1/2.*(y-(fBLS(m1,m2,xmin,xmax,wm))).^2./(wy.*fBLS(m1,m2,xmin,xmax,wm)).^2).*exp(-1/2.*(m1-x).^2./(wm.*x).^2).*exp(-1/2.*(m2-x).^2./(wm.*x).^2));
        for i = 1:length(x)
            Likelihoods(i) = integral2(@(m1,m2)(likefunction(x(i),y(i),m1,m2,xmin,xmax,wm,wy)),xmin-15*wm*xmin,xmax+15*wm*xmax,xmin-15*wm*xmin,xmax+15*wm*xmax);
        end
...
function logL = logLikelihoodTRAPZ(wm,wy,N,m,x,y)
%% LOGLIKELIHOODTRAPZ
%
%   Calculates the loglikelihood of measurement scalar
%   varaibility (wm) and produciton scalar variability (wy), given the
...
switch N
    ...
    case 2
        [M1 M2] = meshgrid(m);
        fBLS = @(m1,m2,xmin,xmax,wm)(integral(@(x)(x.*exp(-1./2.*(x-m1).^2./wm.^2./x.^2).*exp(-1./2.*(x-m2).^2./wm.^2./x.^2)),xmin,xmax,'ArrayValued',true)./integral(@(x)(exp(-1./2.*(x-m1).^2./wm.^2./x.^2).*exp(-1./2.*(x-m2).^2./wm.^2./x.^2)),xmin,xmax,'ArrayValued',true));
        integrand = @(m1,m2,xmin,xmax,wm,wp,x,y)( (1./sqrt(2.*pi.*wy.*fBLS(m1,m2,xmin,xmax,wm))) .* exp( -(fBLS(m1,m2,xmin,xmax,wm)-y).^2./(2.*wp.^2.*fBLS(m1,m2,xmin,xmax,wm).^2) ) .* (1./sqrt(2.*pi.*wm.^2.*x.^2)).^2 .* exp( -( (m1-x).^2. + (m2-x).^2 )./(2.*wm.^2.*x.^2) ) );
...
function logL = logLikelihoodQUAD(wm,wy,N,x,y,xmin,xmax,dx,M,m)
%% LOGLIKELIHOODQUAD
%
...
%     end
    ...
    for i = 1:length(N)
        ...
        p_y_take_fBLS = (1./sqrt(2.*pi.*wy.^2.*fBLS.^2)) .* exp( -(Y - fBLS).^2./(2.*wy.^2.*fBLS.^2) );
...
function logL = logLikelihoodQUADbatch(wm,wy,N,x,y,xmin,xmax,dx,M,m,batchsize)
...
function logL = logLikelihoodQUADnested(wm,wy,N,x,y,xmin,xmax,dx,m,batchsize)
        ...
        for k = 1:ceil(length(x{i})/batchsize)
            ...
            if n == 1
                ...
                for ii = 1:l
                    ...
                    p_y_take_fBLS = (1./sqrt(2.*pi.*wy.^2.*fBLS.^2)) .* exp( -(ytemp - fBLS).^2./(2.*wy.^2.*fBLS.^2) );
