...
%     [numClust, centInd] = decisionGraph(rho, delta, isManualSelect);
    h = figure('Name','Density clustering: rho vs delta');
    plot(rho,delta,'o')
    xlabel('rho')
    ylabel('delta')
    
    deltaCut = input('Delta cutoff?')
    rhoCut = input('Rho cutoff?')
    close(h)
    
    inds = find(delta>deltaCut & rho>rhoCut);
    numClust = length(inds);
    centInd = zeros(NE,1);
    for i = 1:length(inds)
        centInd(inds(i)) = i;
    end
