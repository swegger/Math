function pieceWiseExpRiseFitterTest(varargin)
...
addParameter(Parser,'p',[0,20,100,200])
addParameter(Parser,'sigma',1)
addParameter(Parser,'fitN',100)
addParameter(Parser,'p0',[10*randn,20*randn,200*rand,300*rand])
...
%% Generate data set
T = repmat(t(:),[1,fitN]);
R = repmat(pieceWiseExpRise(t(:),p),[1,fitN]) + sigma*randn(size(T));

%% Fit
for fiti = 1:fitN
    phat(fiti,:) = pieceWiseExpRiseFitter(t(:),R(:,fiti),p0,[Inf,Inf,Inf,Inf],[-Inf,-Inf,0,0]);
    Rhat(:,fiti) = pieceWiseExpRise(t(:),phat(fiti,:));
end
